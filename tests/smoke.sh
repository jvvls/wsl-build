...
assert_file_contains "$INSTALL_SH" 'sudo apt-get install -y openjdk-11-jdk' "Spark instala OpenJDK 11 via APT"
assert_file_contains "$INSTALL_SH" 'local spark_java_home="/usr/lib/jvm/java-11-openjdk-amd64"' "Spark usa JAVA_HOME do tutorial"
assert_file_contains "$INSTALL_SH" 'https://dlcdn.apache.org/spark/spark-${spark_version}/${spark_archive}' "Spark tenta baixar primeiro pelo dlcdn"
assert_file_contains "$INSTALL_SH" 'https://archive.apache.org/dist/spark/spark-${spark_version}/${spark_archive}' "Spark tem fallback para archive Apache"
assert_file_contains "$INSTALL_SH" 'export PATH=\"\$PATH:\$SPARK_HOME/bin\"' "Spark exporta PATH alinhado ao tutorial"
assert_file_contains "$INSTALL_SH" 'export SPARK_LOCAL_IP=127.0.0.1' "Spark fixa IP local para execucao em WSL"
assert_file_contains "$INSTALL_SH" 'export SPARK_LOCAL_HOSTNAME=localhost' "Spark fixa hostname local para execucao em WSL"
assert_file_contains "$INSTALL_SH" 'upsert_shell_block "$HOME/.bashrc" "WSL DEV ENV - SPARK"' "Spark atualiza bloco existente no bashrc"
...
assert_file_contains "$README" 'JAVA_HOME=/usr/lib/jvm/java-11-openjdk-amd64' "README documenta JAVA_HOME do Spark"
assert_file_contains "$README" 'SPARK_LOCAL_IP=127.0.0.1' "README documenta IP local do Spark"
...
assert_file_contains "$VALIDATE_WSL_SH" 'JAVA_HOME aponta para Java 11' "validador WSL confere Java 11 pelo JAVA_HOME"
